...
void S21Matrix::minus_row_col(const S21Matrix& origin_matrix, S21Matrix& temp_matrix,
                   int row, int col) {
  int miss_row = 0;
...
S21Matrix S21Matrix::CalcComplements() {
  ...
  for (int i = 0; i < temp_matrix._rows; ++i) {
    for (int j = 0; j < temp_matrix._cols; ++j) {
      ...
      minus_row_col(*this, additions_matrix, j, i);
      if ((i + j) % 2 == 0) {
        sign = 1;
      } else {
        sign = -1;
...
S21Matrix S21Matrix::InverseMatrix() {
  double temp_det = this->Determinant();
  if (temp_det < DELTA) {
  ...
  if (_cols == 1) {
    ...
    temp_matrix(0, 0) = 1 / (*this)(0, 0);
    return temp_matrix;
  }

  S21Matrix complements = CalcComplements();
  S21Matrix temp_matrix = complements * (1 / temp_det);
  return temp_matrix;
}
